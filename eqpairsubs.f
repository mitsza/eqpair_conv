...
c     ===================================
      SUBROUTINE EQPAIRC(Ear,Ne,Param,Ifl,Photar,Photer)
...
c     3: non-thermal-to-hard compactness ratio (0 for pure thermal plasma)
c     4: Thomson optical depth of ionization electrons (e.g., 0)
c     5: source region radius in cm (for Coulomb/bremsstrahlung only)
c     6: minimum Lorentz factor for power law injection (not used for mono)
c     7: maximum Lorentz factor for power law injection 
c     8: power law injection index (if <0, inject monoenergetically at #8)
c     9: choose primary pair or electron injection (0=electron, 1=pair)
c     10: f_sc
c
c    Note: if #3 only show BB
c
c     algorithm:
...
c
      ...
      INTEGER Ne,Ifl,i,j,ie
      INTEGER,PARAMETER:: np=10
      
      REAL Param(np),Ear(0:Ne),Photar(Ne),Photer(Ne)

      INTEGER imodel
      DOUBLE PRECISION compspect(NPHENERG),blbdspect(NPHENERG)
      DOUBLE PRECISION totspect(NPHENERG)
      DOUBLE PRECISION wkev(NPHENERG)
      DOUBLE PRECISION dphotar(Ne)
      DOUBLE PRECISION :: xa(10),ya(10),x,y,dy
      DOUBLE PRECISION :: tphotard(NPHENERG)
      REAL tphotar(NPHENERG)
      LOGICAL,SAVE::FIRSTCALL=.True.
      DOUBLE PRECISION,SAVE::tear(0:NPHENERG)
      ...
      IF(FIRSTCALL)THEN
         do ie=1,NPHENERG
            tphotar(ie)=ie
         enddo
         call mspairiter(Param,np,imodel,compspect,blbdspect,wkev,
     &        tphotar)
         tear(0) = wkev(1)*10.**(-0.05d0)
         do i=1,NPHENERG
            tear(i)=wkev(i)
         enddo
      FIRSTCALL=.False.
      ...
      call map_to_grid(tear,NPHENERG,1d0*ear,ne,tphotard,1.d0*photar)

      do i=1,NPHENERG
         tphotar(i)=REAL(tphotard(i))
!         write(*,*)i,wkev(i),tphotar(i)
      enddo
...
c      do i=1,NPHENERG
c         write(*,*)i, wkev(i),compspect(i),blbdspect(i)
c      enddo


      do i=1,NPHENERG
         totspect(i)=compspect(i)+blbdspect(i)
      enddo
      
c      tear(0) = wkev(1)*10.**(-0.05d0)
c      do i=1,NPHENERG
c         tear(i)=wkev(i)
c         totspect(i)=totspect(i)*(tear(i)-tear(i-1))
c      enddo

      dphotar=1.d-30

      DO ie = 1, ne
         if((ear(ie).gt.wkev(NPHENERG)).or.(ear(ie).lt.wkev(1)))then
            dphotar(ie)=1.d-25
         else
            i = 3
            do while(ear(ie).gt.wkev(i))
               i = i+1
            enddo
            do j=1,4
               xa(j) = dlog10(wkev(i - 3 + j))
               ya(j) = dlog10(totspect(i - 3 + j))
            enddo  
            x = log10(ear(ie))
...
c      call map_to_grid(1d0*ear,ne,tear,NPHENERG,dphotar,totspect)
      
      IF(param(10).gt.0.0)THEN
         do i=1,ne
            photar(i)=(1.0-param(10))*photar(i)+
     &           param(10)*REAL(dphotar(i))*(ear(i)-ear(i-1))
!     write(*,*)i,photar(i)
         enddo
      ELSE
         do i=1,ne
            photar(i)=
     &           abs(param(10))*
     &           (REAL(dphotar(i))*(ear(i)-ear(i-1))-photar(i))
         enddo
      ENDIF

      do i=1,ne
         photar(i)=max(0.0,photar(i))
         IF(photar(i).ne.photar(i))write(*,*)i,photar(i)
      enddo

c      open(unit=2,file='eqpc.dat',status='new')
c      do i=1,NPHENERG
c         write(2,*)i, wkev(i),compspect(i),blbdspect(i)
c      enddo
c      close(2)

     
      END SUBROUTINE EQPAIRC
     


      SUBROUTINE mspairiter(param,np,imodel,compspect,
     &                    blbdspect,wkev,tphotar)
...
c set model parameters and initialize model
      ...
      call mssetup(param,np,imodel,radnorm,tphotar)
...
c Calls routines dotherm, dobrem, mkscat
      ...
      SUBROUTINE msdopcomp(isthermonly)
...
c call annihilation + brem + anything else that makes photons
      ...
      do i=1,NPHENERG
         gder(i) = 0.d0
      enddo
      call msdotherm(isthermonly)
      IF ( .NOT.isthermonly ) call msdobrem     
...
c Calls routines getethd, dotherm, docoul, dobrem
      ...
      SUBROUTINE msgetth(isthermonly, ein1, posin1, psum)
      IMPLICIT NONE
      INCLUDE "mscommon.eq" 
...
c      if (iread.gt.0) then
      call msgetethd(isthermonly, eout2)
      IF ( isthermonly ) THEN
         elecdens = taup
         posdens = 0.d0
      ELSE
...
c ibrem(61)
      ...
      SUBROUTINE msifitsread(lun,hdu,dummy,naxis,nax3,nax2,nax1,status)
      IMPLICIT NONE
...
c     print *,'nel i',nelements, nelements*4
     ...
     &     dummy, anyf, status)
      ...
      SUBROUTINE msfitsread(lun,hdu,dummy,naxis,nax3,nax2,nax1,status)
      IMPLICIT NONE
...
c=================================
      SUBROUTINE mssetup(param,np,imodel,radnorm,tphotar)
      IMPLICIT NONE
      INCLUDE "mscommon.eq"
      ...
      DOUBLE PRECISION b0, rntot, tbb, temp
...
c      print *,'in setup' 
      IF ( imodel .EQ. 1 ) THEN
         rle = param(1)*param(2)*param(3)
         rls = param(2)
         rlth = param(1)*param(2)*(1-param(3))
      ELSE IF ( imodel .EQ. 2 ) THEN
         rle = 0.0d0
         rls = param(2)
         rlth = param(1)*param(2)*(1-param(3))
...
c blackbody temperature in m_ec^2
      ...
      igmax = max(min(int(log10(param(7))/.05d0+1.5d0),NLORGAM),2)
      plawin = param(8)
      igmin = 1
      if (plawin .gt. 0.d0) then
         igmin = max(min(int(log10(param(6))/.05d0+1.5d0),NLORGAM),2)
      endif
      if (param(9).gt.0) then
         qpairinj = .true.
      else
         qpairinj = .false.
      endif
...
c  black body photon injection
      if (rls .ne. 0.) then
         if (bbtemp .lt. 0.) then 
c         use diskpn model
c            tparam(1) = SNGL(-bbtemp*511.0)
c            tparam(2) = SNGL(Rin)
            do i=1,NPHENERG
               tear(i) = SNGL(upwgam(i)*511.0)
            enddo
            tear(0) = SNGL(upwgam(1)*10.**(-0.05d0)*511.0)
c            call xsdiskpn(tear,NPHENERG,tparam,1,tphotar,tphoter)
            do i=1,NPHENERG
               phinjfn(i) = tphotar(i)
c               write(*,*)i, phinjfn(i)
            enddo
...
c Calls routines doelec, dopcomp
      ...
      SUBROUTINE  mssolve(isthermonly)
      IMPLICIT NONE
...
c   i/r: gder
      ...
      SUBROUTINE  msthscat(rate,eav,disp)
      IMPLICIT NONE
...
c Calls routine thscat.
      ...
      SUBROUTINE msdotherm(isthermonly)
      IMPLICIT NONE
      LOGICAL isthermonly
      INCLUDE "mscommon.eq"
 ...
 233  continue
      ...
      IF ( .NOT.isthermonly ) THEN
         ...
         if (theta .lt. 0.005) then
            ...
            call msthscat(2.d0*rate,eav,disp)
         else
c use Svensson (83)
            ...
            do 510 i = 80+IOFF,142+IOFF
               ...
               rl =  msannspme(wtarg,theta)
               wtarg = rlb + gwidth(i)*xu
               ru = msannspme(wtarg,theta)
c    AZ msannspme gives answer for unit opt. depth     
...
c       elecdens, posdens
      ...
      SUBROUTINE msmkscat(pout, scat, iscat)
      IMPLICIT NONE
      INCLUDE "mscommon.eq"
...
c   i/r: ethder
      ...
      SUBROUTINE msgetethd(isthermonly, eout2)
      IMPLICIT NONE
      INCLUDE "mscommon.eq"
...
c Local variables
      ...
      DOUBLE PRECISION x, xl, xp, xu, zrate, msannspme
...
c    r: gdot, ethder
      ...
      SUBROUTINE msdoelec(isthermonly)
      IMPLICIT NONE
      INCLUDE "mscommon.eq" 
...
c get thomson optical depth
      call msgetth(isthermonly, ein(1), posin(1), psum)
...
c      print *,'e- time=',time(1)
      ...
      double precision function msannspme(x, temp)
...
c  3/32/pi follow from assuming n+=n- and using sigma(Thomson)
      msannspme=a1*a2*cc/temp*3/32./pi
      return
      end
...
c         elseif (theta .ge. .16) then
 ...
 399  continue
      ...
      subroutine map_to_grid(new_en,nne,old_en,one,new_flu,old_flu)
      implicit none
      integer :: i, j, one, nne, bmin, bmax
      double precision :: new_en(0:nne), new_flu(nne)
      ...
      do i=1,nne
         ...
         if((new_en(i).le.old_en(0)).or.
     &        (new_en(i-1).ge.old_en(one)))then
         new_flu(i) = 0.d0
      else
         call dbinsrch(new_en(i-1),bmin,old_en,one+1)
         call dbinsrch(new_en(i),bmax,old_en,one+1)
         bmin = bmin-1
         bmax = bmax-1

         if (bmin.eq.bmax) then
            new_flu(i) = (new_en(i)-new_en(i-1))/
     &           (old_en(bmin+1)-old_en(bmin)) 
     &           *old_flu(bmin+1)
         else
            dmin=(old_en(bmin+1)-new_en(i-1))/
     &       (old_en(bmin+1)-old_en(bmin))
            dmax=(new_en(i)-old_en(bmax))/(old_en(bmax+1)-old_en(bmax))

            new_flu(i)=new_flu(i)+old_flu(bmin+1)*dmin
            do j=bmin+2,bmax
               new_flu(i)=new_flu(i)+old_flu(j)
            enddo
            new_flu(i)=new_flu(i)+old_flu(bmax+1)*dmax
         endif
      endif
      enddo

      end subroutine map_to_grid

      subroutine dbinsrch(e,k,ener,n)
...
!     adapted from J. Wilms's tbvabs_new.f routine                                                                                                                                                                                   
 ...
 1    if (khi-klo.gt.1) then
      ...
      end subroutine dbinsrch


      SUBROUTINE polint(xa,ya,n,x,y,dy)
!  this program is working for the Lagrange polynomial interpolation,
!  input n pairs of (xa,ya) and x at which you want to know y. After calling polint, 
!  return a value of y and an error estimate as the output.
      INTEGER n,NMAX
      DOUBLE PRECISION dy,x,y,xa(n),ya(n)
      PARAMETER (NMAX=10) ! largest number of pairs of data (xa,ya). Change as you need
      INTEGER i,m,ns
