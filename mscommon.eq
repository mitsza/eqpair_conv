...
c      PARAMETER (NPHENERG=400, NLORGAM=81)
...
c Used in dopcomp, getth.
      ...
      COMMON /mseqpbigdata/ av, ppr, ppf, ipp
      COMMON /mseqpbrempar/ brem, brema, ibrem
      COMMON /mseqpccoul/ radius
      COMMON /mseqpchatter/ diff,dt,rle,rls,rlth,iter
      COMMON /mseqpchat2/ bbtemp,prninj,erninj,taup,dothermpairs
      COMMON /mseqpdist/ edist, gdist, gder, esc, theta, eth, 
     &                 ethder, posdist, elecdens, posdens
      COMMON /mseqpeminus/ gdot, idump
      COMMON /mseqphthrm/ htcr, htcav, htcd, htcavl, htcdl
      COMMON /mseqpiterpar/ niter
      COMMON /mseqplotcmp/ cltr, clta, cltd
      COMMON /mseqpparams/ pairinjfn, epinjfn, phinjfn,
     &                   een, wgam
      COMMON /mseqpscatter/ upwgam, gwidth
      COMMON /mseqpthermal/ thann, cr
      COMMON /mseqpthstuff/ tesc, otesc
      COMMON /mseqplccomp/ clav, rldisp, ig
      COMMON /mseqpbigdo/ ecomp, gcomp, iecomp, igcomp
      COMMON /mseqpcompton/ compr, ictab, iectab, ctab, ectab
      COMMON /mseqpxflags/ qpairinj
